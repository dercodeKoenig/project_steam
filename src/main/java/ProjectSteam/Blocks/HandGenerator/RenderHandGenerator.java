...
public class RenderHandGenerator implements BlockEntityRenderer<EntityHandGenerator> {
    ...
    public void render(EntityHandGenerator tile, float partialTick, PoseStack stack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {
        ...
        if (axleState.getBlock() instanceof BlockHandGenerator) {
            ...
            m2 = m2.rotate(new Quaternionf().fromAxisAngleDeg(0f, (float) 0f, 1.0f, (float) (rotorRotationMultiplier*( tile.getMechanicalData().currentRotation+tile.getMechanicalData().internalVelocity*partialTick))));
            ...
            m2 = m2.rotate(new Quaternionf().fromAxisAngleDeg(0f, (float) 1f, 0f, (float) (rotorRotationMultiplier*( tile.getMechanicalData().currentRotation+tile.getMechanicalData().internalVelocity*partialTick))));
            shader.setDefaultUniforms(VertexFormat.Mode.TRIANGLES, m2, RenderSystem.getProjectionMatrix(), Minecraft.getInstance().getWindow());
            shader.getUniform("NormalMatrix").set(new Matrix3f(m2).invert().transpose());
