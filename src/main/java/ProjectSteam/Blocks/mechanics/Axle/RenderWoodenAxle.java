...
public class RenderWoodenAxle implements BlockEntityRenderer<EntityWoodenAxle> {
    ...
    @Override
    public void render(EntityWoodenAxle tile, float partialTick, PoseStack stack, MultiBufferSource bufferSource, int packedLight, int packedOverlay) {

        BlockState axleState = tile.getBlockState();
        if (axleState.getBlock() instanceof BlockWoodenAxle) {
            Direction.Axis facingAxis = axleState.getValue(BlockWoodenAxle.ROTATION_AXIS);
